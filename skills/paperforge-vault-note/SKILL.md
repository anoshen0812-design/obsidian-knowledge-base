...
5. Use the PaperForge section order required by `system/workflows/ingest.md`. Keep the original bibliographic title in frontmatter and use `display_title` for the filename, level-one heading, and index alias. Copy `impact_factor`, `impact_factor_year`, and `impact_factor_source` exactly from `TASK_JSON`; keep each property present with YAML `null` when metadata is unavailable, and never estimate or substitute another citation metric.
